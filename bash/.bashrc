...
if [ "$color_prompt" = yes ]; then
    ...
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\W\[\033[38;5;36m\]$(__git_ps1)\[\033[00m\]\$ '
else
...
# bash shorcuts
if [ -f ~/.config/shortcuts.bash ]; then
    . ~/.config/shortcuts.bash
fi

# git prompt
if [ -f ~/.config/git-sh-prompt ]; then
    . ~/.config/git-sh-prompt
fi
...
# Created by `pipx` on 2023-08-19 04:29:08
...
# enable fzf keyboard shortcuts if fzf is installed
if [ -x "$(command -v fzf)" ]
then
